...
## 2026-05-02 渲染失败必须清理所有结果面而不是只清内部状态

- **错误**：缓冲块吸能页 `_on_calculate()` 捕获 `_render_result()` 后段异常时，初版只清了 `_last_result/_last_payload` 并禁用导出；但指标卡、总体结论、结果文本、工作台状态列已经写入“通过 / 最大压缩”等成功态，用户会看到半成功界面。
- **正确做法**：计算失败、渲染失败、输入变更、加载输入条件都必须走同一个失效路径：清空指标卡、结果文本、报告预览、badge、参数对比表、时域图，并把工作台状态恢复为“待仿真”；如果曲线仍有效，只保留“已导入曲线”的事实，不保留任何上一次计算结果。
- **原因**：UI 的可信度来自“当前显示状态”和“内部状态”一致。只清内部变量会让用户看到旧成功结论，但导出被禁用、内部结果为空，形成最危险的决策误导。
