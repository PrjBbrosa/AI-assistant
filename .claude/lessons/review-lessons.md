...
## 2026-05-02 二次 review 要复现用户可见残留而不是只看测试绿

- **错误**：缓冲块吸能模块第一次修复渲染失败后，focused/full 测试全绿，但 UI reviewer 继续用 headless smoke 模拟“报告预览在渲染末尾失败”，发现 `workbench_status_label` 仍残留“最大压缩 / 状态: 通过”。
- **正确做法**：review 提出 UI 状态类问题后，recheck 必须复现“用户可见面板是否仍有残留”，不能只看内部状态和测试结果。尤其要逐个检查新增结果面：指标卡、状态 rail、文本框、图表、导出预览、工作台摘要。
- **原因**：状态残留类 bug 很容易被局部测试漏掉。二次 review 的价值就是站在用户视角找“还有哪块屏幕没被清”，否则修复会停在内部变量层面。

## 2026-07-02 lessons 修复模式必须全模块 grep 巡检推广

- **错误**：Goodman 无下限、三态校核、导出失效、渲染保护、状态清理五个模式都已在某个模块正确实现，但从未回填其余模块，2026-07-02 review 发现全部存在“部分模块回归”。
- **正确做法**：每次把教训修进某个模块时，立即 grep 全仓找同类反模式（如 `max(0.1`、`all(checks`、`_render_result` 在 try 外），把命中清单挂进 plan 逐一处理或显式记录豁免原因。
- **原因**：“教训只对写代码时在场的模块生效”是系统性风险；一次 grep 的成本远低于两个月后的全面 review。
- **补充（2026-07-02 二次校核）**: 消费点清单必须包含全部渲染出口（UI 徽章 / 文本报告 / 富 PDF / CLI），逐出口写断言；本轮 overall_status 三个出口只测了两个，漏的正是会被存档外发的 PDF。
