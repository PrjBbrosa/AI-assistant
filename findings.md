...
## 2026-03-22 Spline Fit Source Search Notes

### Public Source Availability
- 公开且足够权威的 DIN 5480 “完整计算案例”很少；多数真正可复核的案例仍在付费标准、商业软件报告或 FVA 环境里
- 当前最接近工程可用的公开组合来源是：
  - DIN Media 标准入口页（确认标准身份与版本）
  - GWJ eAssistant 官方 handbook（公开说明几何与强度链）
  - FVA Workbench KnowledgeBase（公开说明 DIN 5480 / DIN 5466 / FVA 591 关系）
  - Mädler / Grob 等公开标准件目录（提供小规格 DIN 5480 真实尺寸）

### Geometry Findings
- GWJ eAssistant handbook 明确说明：DIN 5480 是“based on reference diameters”，并使用 `reference diameter d_B`、`module`、`tooth count z`、`profile shift` 等参数选择/反算几何
- 这直接否定了当前 `d = m * z` 即可代表 DIN 5480 参考直径的简化假设
- 对 10~15 mm 直径范围，公开标准件目录中已找到 `W 15 x 1.25 x 10` / `N 15 x 1.25 x 10` 这类真实小规格
- Mädler 公开目录给出 `W 15 x 1.25 x 10` 的外径约 `14.75 mm`、根径约 `12.1 mm`，与当前模块自推公式明显不同
- 另一个公开存在的小规格接口是 `W14x0.8x16`，见 Eppinger/Sauter 机床接口资料，但未找到同等级公开尺寸表

### Strength Findings
- eAssistant handbook 公开说明其 involute spline strength 按 `Niemann Maschinenelemente Vol. 1, 2001` 计算
- 该 handbook 同时说明 load distribution / share factor、support factor、hardness factor、operating / peak load safety 等都会进入强度链
- FVA Workbench KnowledgeBase 公开说明：
  - geometry according to `DIN 5480 Part 1 (2006)`
  - load capacity according to `DIN 5466 Part 1 (2000)` with extensions from `FVA project 591`
- 这说明如果要把模块升到“工程校核候选”，不能只保留一个 `k_alpha` 与一个人工 `p_zul`

### Version Notes
- DIN Media 官方页面显示 `DIN 5480-1:2006-03` 已被 `DIN 5480-1:2025-10` 替代
- 公开 handbook / FVA 页面当前仍大量引用 2006 版 DIN 5480 与 2000 / 2012 版 DIN 5466 / DIN 6892 体系
- 因此后续如果要做“标准化重构”，需要明确我们对齐的是哪一版标准链，不能只写“DIN 5480”

### Immediate Recommendation
- Stage B 不应继续做“假标准化”的自由推导
- 更稳妥的下一步是：
  - 先把场景 A 改成 `reference diameter + module + z + optional profile shift/source`
  - 用 `W15 x 1.25 x 10` 作为公开小规格 geometry benchmark
  - 把 `W14x0.8x16` 只作为“存在的小直径工业接口示例”，暂不作为精确 benchmark

### Implementation Outcome In Worktree
- `core/spline/geometry.py` 已改为两条路径：
  - `explicit_reference_dimensions`
  - `approximation_from_module_and_tooth_count`
- 近似模式现在必须显式标记，并返回 warning / source trace
- 场景 A 结果已显式返回：
  - `overall_verdict_level = simplified_precheck`
  - `model_assumptions`
  - `not_covered_checks`
- `W/N 15 x 1.25 x 10` 已接入：
  - geometry 单元测试
  - calculator 预校核测试
  - `examples/spline_case_01.json`

### Re-review Conclusion After Stage B
- 当前模块已明显优于初版：
  - 不再把近似几何伪装成标准几何
  - 有公开来源与 benchmark
  - verdict 已显式降级为 `simplified_precheck`
- 但它仍然**不是**正式工程校核链，原因包括：
  - 仍未实现 DIN 5466 / DIN 6892 全部强度与峰值载荷链
  - 仍缺 support factor / hardness factor / peak load / reversing load 等关键项
  - 仍未接入标准正文级 tolerance / profile shift / flank clearance 解析
- 因此当前最准确的定位仍是：
  - `可追溯的简化预校核`
  - `不适合作为正式工程签发校核`
