...
torque_reserve_bonus   ∈ {0..3}   （S_torque ≤ 1.2/1.5/2.0/>2.0 → 3/2/1/0）
combined_reserve_bonus ∈ {0..3}   （S_combined 分开独立加分）
...
**扭矩储备与联合作用储备是两项独立加分项**（各自按 S 值查表扣分），不是合并的 slip_reserve。实现见 `core/interference/fretting.py:148-217`。
...
`not_applicable` 意味着当前过盈配合的某些前置条件不满足简化模型（例如**任何非零弯矩**、空心轴组合、模量差异过大）；此时 Fretting 评估被视为"超出简化范围"，需要手工判断。`core/interference/fretting.py:101-103` 对 `has_bending` 为真时一律返回 not_applicable，不做"显著弯矩"阈值判断。
