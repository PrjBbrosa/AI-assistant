"""Paint-only chapter navigation delegate.

Draws a number tile plus the step title. Item text, tooltip, and model
data stay unchanged; this class does not create a QWidget per row.
"""
...
_ITEM_TEXT_RE = re.compile(r"^步骤\s+(\d+)\.\s+(.*)$")
...
def parse_chapter_item_text(text: str) -> tuple[str, str]:
    """Split ``步骤 N. title`` into a zero-padded tile label and the title."""
...
class ChapterNavigationDelegate(QStyledItemDelegate):
    """Paint step number tile + title; hover/selected colors from tokens."""
    ...
    def label_text_rect(self, option: QStyleOptionViewItem, index: QModelIndex) -> QRect:
        """Text rect used for the chapter title (excludes the number tile)."""
        del index
    ...
    def elided_label(self, option: QStyleOptionViewItem, index: QModelIndex) -> str:
        """Return the title string that ``paint`` would draw."""
        _tile, name = parse_chapter_item_text(str(index.data(Qt.ItemDataRole.DisplayRole) or ""))
    ...
    def paint(
        ...
        tile_text, name = parse_chapter_item_text(raw)
