...
class BufferEnergyCurveWidget(QWidget):
    ...
    def curve_data(
        self,
    ) -> tuple[list[tuple[float, float]], list[tuple[float, float]], float, float, float, bool]:
        """Stored F-x series and limit markers consumed by paintEvent."""
        return (
            list(self._loading),
            list(self._unloading),
            self._x_max_mm,
    ...
    def paintEvent(self, _event) -> None:  # noqa: N802
        ...
        if self._bottom_out and self._available_stroke_mm > self._x_max_mm:
            ...
            painter.fillRect(QRectF(x0, plot.top(), max(0.0, x1 - x0), plot.height()), _token("fail_fg", 72))

        self._draw_hysteresis_fill(painter, to_px)
        self._draw_polyline(painter, self._loading, to_px, QPen(_token("accent"), 2.2))
        self._draw_polyline(
            painter,
            self._unloading,
            to_px,
            QPen(_token("secondary"), 2.0, Qt.PenStyle.DashLine),
        )
    ...
    def _draw_hysteresis_fill(self, painter: QPainter, to_px) -> None:
        ...
        for point in reversed(self._unloading):
            path.lineTo(to_px(*point))
        path.closeSubpath()
        painter.fillPath(path, _token("accent", 46))
    ...
    def _draw_limits(self, painter: QPainter, plot: QRectF, to_px) -> None:
        painter.setPen(QPen(_token("fail_fg"), 1, Qt.PenStyle.DotLine))
    ...
    def _draw_marker(self, painter: QPainter, to_px) -> None:
        ...
        painter.setPen(qpen("ink_primary", 1.2))
        painter.setBrush(_token("ink_primary"))
    ...
    def _draw_labels(self, painter: QPainter, plot: QRectF, max_x: float, max_f: float) -> None:
        ...
        painter.setPen(QPen(_token("secondary"), 1.8, Qt.PenStyle.DashLine))
