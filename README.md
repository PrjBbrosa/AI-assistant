...
- `疲劳强度与可靠性`：S-N/Wöhler 试验拟合、载荷谱换算、Miner 损伤与概率可靠性预校核
...
`requirements.txt` 只声明下限；`constraints.txt` 钉死当前已验证可 import 的精确版本（PySide6、PyInstaller、reportlab、matplotlib、openpyxl、numpy、scipy，以及测试用的 pytest）。更新依赖时先 `pip install -r requirements.txt`，再用环境里实际安装的版本刷新 `constraints.txt` 的 `==` 行。测试额外依赖见 `requirements-dev.txt`：
...
Windows runner 会执行 PyInstaller onedir 构建、构建信息校验和 EXE 10 秒启动 smoke；只有对应 CI run 实际通过时，才能视为该提交通过了 Windows 浅层门禁。八模块计算、保存/加载和报告导出仍需按 `scripts/windows_smoke.md` 做真实桌面人工验收。
...
| 疲劳强度与可靠性 | `首版预校核` | 支持金属、非增强工程塑料和短纤增强塑料；覆盖删失 S-N 拟合、rainflow/块谱、Miner 损伤和 Monte Carlo 可靠性，不覆盖应变寿命、多轴或连续纤维层合材料 |
